...
from passivbot_jit import calc_reentry_qty, calc_diff, calc_long_reentry_price, calc_shrt_reentry_price, \
                          sort_dict_keys, calc_initial_entry_qty, calc_shrt_closes, calc_long_closes


#from passivbot import

from passivbot_jit import round_up, round_dn, round_

from bybit_jit import create_bot as create_bot_bybit
from bybit_jit import fetch_trades as bybit_fetch_trades
from bybit_jit import calc_cross_long_liq_price as bybit_calc_cross_long_liq_price
from bybit_jit import calc_cross_shrt_liq_price as bybit_calc_cross_shrt_liq_price
from bybit_jit import calc_isolated_long_liq_price as bybit_calc_isolated_long_liq_price
from bybit_jit import calc_isolated_shrt_liq_price as bybit_calc_isolated_shrt_liq_price



from binance_jit import create_bot as create_bot_binance
from binance_jit import fetch_trades as binance_fetch_trades
from binance_jit import calc_cross_long_liq_price as binance_calc_cross_long_liq_price
from binance_jit import calc_cross_shrt_liq_price as binance_calc_cross_shrt_liq_price
from binance_jit import calc_isolated_long_liq_price as binance_calc_isolated_long_liq_price
from binance_jit import calc_isolated_shrt_liq_price as binance_calc_isolated_shrt_liq_price
...
@njit
def calc_liq_price(balance_, pos_size_, pos_price_, leverage_, inverse, cross_mode):
    if inverse:

        if cross_mode:
            if pos_size_ > 0:
                liqprice = bybit_calc_cross_long_liq_price(balance_, pos_size_, pos_price_, leverage_)
            else:
                liqprice = bybit_calc_cross_shrt_liq_price(balance_, pos_size_, pos_price_, leverage_)

        else:
            if pos_size_ > 0:
                liqprice = bybit_calc_isolated_long_liq_price(balance_, pos_size_, pos_price_, leverage_)
            else:
                liqprice = bybit_calc_isolated_shrt_liq_price(balance_, pos_size_, pos_price_, leverage_)
    else:
        if cross_mode:
            if pos_size_ > 0:
                liqprice = binance_calc_cross_long_liq_price(balance_, pos_size_, pos_price_, leverage_)
            else:
                liqprice = binance_calc_cross_shrt_liq_price(balance_, pos_size_, pos_price_, leverage_)

        else:
            if pos_size_ > 0:
                liqprice = binance_calc_isolated_long_liq_price(balance_, pos_size_, pos_price_, leverage_)
            else:
                liqprice = binance_calc_isolated_shrt_liq_price(balance_, pos_size_, pos_price_, leverage_)

    return liqprice


@njit
def calc_cost(qty_, price_, inverse):
    return qty_ / price_ if inverse else qty_ * price_


@njit
def calc_initial_entry_qty_(min_qty_, min_notional_, qty_step_, balance_, last_price, leverage_, entry_qty_pct_,
                            inverse):
    if inverse:
        return calc_initial_entry_qty(min_qty_, qty_step_, balance_ * last_price * leverage_, entry_qty_pct_)
    else:
        return calc_initial_entry_qty(max(min_qty_, round_up(min_notional_ / last_price, qty_step_)),
                                      qty_step_, (balance_ / last_price) * leverage_,
                                      entry_qty_pct_)


@njit
def calc_max_pos_size(balance_, price_, leverage_, inverse):
    return balance_ * price_ * leverage_ if inverse else balance_ / price_ * leverage_



@njit
def calc_long_reentry_price_(balance_, pos_margin_, pos_price_, highest_bid_, price_step, grid_spacing, grid_coefficient):
    return min(highest_bid_, calc_long_reentry_price(price_step, grid_spacing, grid_coefficient, balance_,
                                                     pos_margin_, pos_price_))


@njit
def calc_shrt_reentry_price_(balance_, pos_margin_, pos_price_, lowest_ask_, price_step, grid_spacing, grid_coefficient):
    return max(lowest_ask_, calc_shrt_reentry_price(price_step, grid_spacing, grid_coefficient,
...
def backtest(trades_list, settings):
    ...
    entry_qty_pct = settings['entry_qty_pct']
    ...
    eleverage = settings['leverage']

    if inverse:

        if settings['cross_mode']:
            liqleverage = max_leverage
        else:
            liqleverage = leverage
    ...
    '''
    for e in settings['break_on']:
        if e[0].startswith('ON:'):
            break_on[e[0]] = eval(e[1])
    '''
    ...
    for t in trades_list:
        ...
        if t['buyer_maker']:
            ...
            if pos_size == 0.0:
                ...
                if do_long:
                    ...
                    bid_qty = calc_initial_entry_qty_(min_qty, min_notional, qty_step, apparent_balance, ob[0],
                                                      eleverage, entry_qty_pct, inverse)
                else:
                    bid_price = 0.0
                    bid_qty = 0.0
            elif pos_size > 0.0:
                if calc_diff(liq_price, ob[1]) < stop_loss_liq_diff and t['price'] <= liq_price:
                    # long liq
                    print('break on long liquidation, liq price: ', liq_price)
                    return []
                # long reentry
                bid_qty = calc_reentry_qty(qty_step, ddown_factor,
                                           calc_initial_entry_qty_(min_qty, min_notional, qty_step, apparent_balance,
                                                                   ob[0], eleverage, entry_qty_pct, inverse),
                                           calc_max_pos_size(apparent_balance, ob[0], eleverage, inverse),
                                           pos_size)

                if bid_qty >= max(min_qty, min_notional / t['price']):
                    pos_margin = calc_cost(pos_size, pos_price, inverse) / leverage
                    bid_price = calc_long_reentry_price_(apparent_balance, pos_margin, pos_price,
                                                         ob[0], price_step, grid_spacing, grid_coefficient)
                else:
                    bid_price = 0.0
            else:
                # short pos
                if calc_diff(liq_price, ob[0]) < stop_loss_liq_diff or \
                    ...
                    if t['price'] <= pos_price:
                        # short close
                        init_entry = calc_initial_entry_qty_(min_qty, min_notional, qty_step, apparent_balance, ob[0],
                                                             eleverage, entry_qty_pct, inverse)
                        min_close_qty = max(
                            min_qty,
                            round_dn(init_entry * min_close_qty_multiplier,
                                     qty_step)
                        )
                        qtys, prices = calc_shrt_closes(price_step, qty_step, min_close_qty, min_markup, max_markup, pos_size, pos_price, ob[0], n_close_orders)
                        if len(qtys) > 0:
                            bid_qty = qtys[0]
                            bid_price = prices[0]
                        else:
            ...
            if t['price'] < bid_price and bid_qty >= min_qty:
                ...
                qty = bid_qty
                price = bid_price
                cost = calc_cost(bid_qty, bid_price, inverse)
                pnl = -cost * maker_fee
                if pos_size >= 0.0:
                ...
                apparent_balance = actual_balance * balance_pct
                pnl_sum += pnl
                liq_price = calc_liq_price(actual_balance, pos_size, pos_price, liqleverage, inverse, cross_mode)
                append_trade = True
        else:
            # sell
            if pos_size == 0.0:
                ...
                if do_shrt:
                    ...
                    ask_qty = -calc_initial_entry_qty_(min_qty, min_notional, qty_step, apparent_balance, ob[1],
                                                       eleverage, entry_qty_pct, inverse)
                ...
                if calc_diff(liq_price, ob[1]) < stop_loss_liq_diff and t['price'] >= liq_price:
                    # shrt liq
                    print('break on shrt liquidation, liq price: ', liq_price)
                    return []
                # shrt reentry
                ask_qty = -calc_reentry_qty(qty_step, ddown_factor,
                                            calc_initial_entry_qty_(min_qty, min_notional, qty_step, apparent_balance,
                                                                    ob[1], eleverage, entry_qty_pct, inverse),
                                            calc_max_pos_size(apparent_balance, ob[1], eleverage, inverse),
                                            pos_size)
            ...
            if t['price'] > ask_price and abs(ask_qty) >= min_qty:
                ...
                qty = ask_qty
                price = ask_price
                cost = calc_cost(-ask_qty, ask_price, inverse)
                pnl = -cost * maker_fee
                if pos_size <= 0.0:
        ...
        if append_trade:
            ...
            adg = total_gain ** (1 / n_days_) if n_days_ > 1.0 else 1.0
            trades.append({'trade_id': k, 'side': trade_side, 'type': trade_type,
                           ...
                           'max_pos_size': calc_max_pos_size(apparent_balance, t['price'], eleverage, inverse),
            ...
            apparent_balance = actual_balance * balance_pct
            line = round(progress, 3), "net pnl ", str(np.format_float_positional(pnl_sum, precision=8))
            line += "profit sum ", str(np.format_float_positional(profit_sum, precision=5))
            line += "loss sum ", str(np.format_float_positional(loss_sum, precision=5))
            line += "actual_bal ", round(actual_balance, 4)
            line += "apparent_bal ", round(apparent_balance, 4)
            line += "qty ", round(
                calc_initial_entry_qty_(min_qty, min_notional, qty_step, apparent_balance, ob[0], eleverage,
                                        entry_qty_pct, inverse), 4)
            line += "adg ", round(trades[-1]['average_daily_gain'], 3)
            line += "max pos pct ", round(abs(pos_size) / calc_max_pos_size(apparent_balance, t['price'], eleverage, inverse), 3)
            line += "pos size ", round(pos_size, 4)
            print(line)
...
if __name__ == '__main__':
    ...
    print("[+] Full time elapsed", strftime("%H:%M:%S", gmtime(total_n)))
